...
(if mview-mode-map
  ...
  ;; VI/less like movement(kobayasi)
  (define-key mview-mode-map    "j"     'next-line)
  (define-key mview-mode-map    "k"     'previous-line)
  (define-key mview-mode-map    "h"     'backward-char)
  (define-key mview-mode-map    "l"     'forward-char)
...
; (define-key mview-mode-map 	"q" 	'mview-quit); 簡単に抜けると困る
  ;; どうも、retain を使うか、 auto-kill-flag を使うのが良いらしい
